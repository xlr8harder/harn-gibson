...
The hard-coded `gibson1_renderer.py` is the default calmer visualizer for normal watching. It uses the same renderer contract as every external renderer, but limits itself to a coherent status board, terminal wall, depth-2 repo city with compact child blocks, repo wire terrain, signal scope, trace route, data-rain backdrop, and timeline/route animations. Use it when the goal is to inspect an agent session without the full stress-test scene:
