...
The built-in `tiny-project` trajectory creates an ignored bare workspace under `test-artifacts/dogfood-workspaces/`, injects the long capture prompt from `examples/prompts/dogfood-tiny-project.md`, writes an ignored capture log under `test-artifacts/captures/`, defaults to split fixture conversion, and prints the exact review command when harn exits. For a custom workspace or edited prompt, use the manual form:
