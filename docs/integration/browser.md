...
The browser/canvas display is the first display backend, not the only possible one. Non-web backend experiments can start from `GET /backend-contract` or `uv run harn-gibson backend-contract`, then consume `GET /scene`, `GET /catalog`, and `GET /events/stream` to render scene snapshots and scene-update payloads with their own primitive runtime.
