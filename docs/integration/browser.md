...
Replay fixtures can also render their final scene through the browser display. The browser integration suite renders the checked-in replay fixtures, verifies nonblank canvas metrics, checks non-default style packs, and inspects the `svg_layer` vector-symbol render state used by the primitive gallery:
