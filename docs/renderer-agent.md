...
`examples/renderers/gibson1_renderer.py` is the default calmer hard-coded renderer for normal dogfood watching. It is deterministic, uses real renderer context, and deliberately emits a smaller scene: status, terminal wall, semantic repo city with compact child blocks, repo wire terrain, signal scope, trace route, low-opacity data rain, style-aware tones, timeline/route animations, and subtle object-targeted camera drift/jolts. It consumes `context.project.semanticGraph` when available, adding package/role/degree metadata to city and terrain blocks, filling missing code-file blocks from AST metadata, drawing import/test route links through the trace-route surface, and aiming camera effects at touched city objects through `targetRef`.
