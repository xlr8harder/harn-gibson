...
`svg_layer` is intentionally structured path data, not raw SVG markup. It accepts a `viewBox`, path `d` strings, circles, labels, named gradients, vector-space trace routes, placement, scale, tone, and simple animation hints such as stroke reveal, moving dashes, pulse, spin, gradient paint, and path-trace particles. It rejects the unsafe SVG surface by never inserting model-authored markup into the DOM: no scripts, event handlers, `foreignObject`, or external references. Fuller SVG-style features such as masks, filters, and timed morphs remain future catalog work if they prove useful without making renderer prompts brittle.
