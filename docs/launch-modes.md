...
Add `--playback-timing real-time` to use source timestamp deltas, or `--renderer activity-roll` / `--renderer thermal-roll` / `--renderer classic` / `--renderer stress` to call a built-in visualization live during event replay.
