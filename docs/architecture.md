...
Repo topology is already visual input, not only prompt metadata. The deterministic renderer maps the bounded depth-2 repo sample into both a `node_graph` and a Gibson-style `city_block`: top-level entries become districts, sampled children become smaller nearby blocks, visible file/directory counts drive height, and touched files recolor/focus the matching district with particle, extrusion, and slow `cameraPath` drift effects.
