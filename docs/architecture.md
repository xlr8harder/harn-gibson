...
For a diagram-first overview of the same boundaries, see [Architecture Diagram](architecture-diagram.md).
...
There are three useful presentation extension levels, plus one framework-owned perception layer.

The perception layer decides what the event stream means before a renderer styles it. `RendererContextBuilder` folds normalized harn events and touched-file batches into `harn-gibson.world-model.v1`, an event-sourced model of durable facts. The first version tracks per-file activity, bounded structured change facts from tool payloads, command start/result facts, command status/duration when pairable, touched command paths, test/build health facts inferred from command text with observed outcome status, outcomes with provenance, and lifecycle metadata that classifies rendered entities as current, recent, aging, or stale and as open, observed, or reconciled. The first `harn-gibson.agent-attention.v1` pass infers the current action, objective hint, focus paths/entities, and health focus from the current render batch and world-model facts so renderers can aim a camera or HUD at what the agent appears to be doing. The first `harn-gibson.semantic-repo-graph.v1` pass scans bounded Python AST metadata from the project root to expose package/file/symbol nodes, local import edges, and inferred test-to-code edges without sending file contents. Later perception enrichers should add richer language support, symbol-level call graphs, fact reconciliation policies, and fuller agent plans behind this same renderer-visible contract.

The renderer layer decides what should happen. A renderer consumes normalized harn events, route/coalescing metadata, current scene state, recent visual continuity, project metadata, repo topology, semantic repo graph, touched files, style information, and the visual catalog, then returns a `RenderPlan`. Renderers can be in-process deterministic Python, an external JSON command, a prompt-command model wrapper, or a future provider-backed model adapter. This is the right level for product-specific behavior, different visual personalities, event-routing experiments, and AI-driven scene direction.

The primitive layer decides what a scene can show. Browser primitives and animations are reusable scene vocabulary: `city_block`, `spatial_map`, `terminal_wall`, `signal_scope`, `svg_layer`, `data_rain`, `timeline_cue`, `route_trace`, and the rest of the catalog. A primitive should be browser-local, bounded, deterministic, and parameterized enough that many renderers can use it without authoring per-frame drawing instructions. `spatial_map` is the first deliberately lower-level world-binding primitive: renderers can provide typed objects, edges, stable `entityId` values, focus state, and `worldBindings` instead of choosing a pre-styled set-piece first. This is the right level for adding a new visual toy, richer animation behavior, or a generic display building block.

The display-backend layer decides where pixels or terminal cells are drawn. The browser/canvas backend is the only production backend today, but the scene protocol is not inherently web-only. A native app, terminal renderer, game engine, OpenGL scene, or remote wall display could consume the same scene snapshots and scene-update payloads, then implement the catalog it supports with its own drawing and input loop. `GET /backend-contract` exposes this as `harn-gibson.display-backend-contract.v1`: endpoint paths, scene/update schema names, core primitive kinds, catalog primitive kinds, effect kinds, mutation ops, input delivery modes, render timing modes, supported style packs, active style pack, and the current browser backend capability profile.

The boundary between these layers is `SceneMutation`: renderers produce mutations against primitive props and animation records; display backends own drawing, timing loops, and viewport-specific behavior. Primitive props may also include `worldBindings`, schema `harn-gibson.world-binding.v1`, which declare that a visual property is tied to a durable project/world fact such as file activity, repo topology, command status, health, or lifecycle recency. Bindings are descriptive continuity metadata: renderers, prompt reviews, replay summaries, and alternate backends can use them to preserve meaning, while the browser remains free to render the primitive normally. `spatial_map.objects` can expose those bindings at object level, including opacity bindings for lifecycle fading, and `visualContinuity.objectAnchors` returns exact `targetRef` candidates for `id`, `entityId`, path, label, or index. That lets a user build a new renderer from existing Gibson primitives, add a primitive/effect that deterministic, hard-coded, and AI renderers can all target, or build a non-web display backend that implements the agreed primitive catalog or a declared subset. A custom primitive layer is viable when it either translates the Gibson catalog to backend-native drawing calls or pairs a custom catalog with a renderer that targets that vocabulary.
...
The pipeline also builds a `RendererContext` for renderers that opt into `render_with_context`. The context alternates between full compaction payloads and rolling summaries, combining project metadata, bounded repo topology, semantic repo graph, touched-file summaries, world-model facts, inferred agent attention, catalog entries, current scene state, recent agent context, render intents, recent visualization history, compact visual-continuity anchors, bounded object-level target anchors, and normalized world-binding summaries without requiring a full transcript on each renderer turn.
