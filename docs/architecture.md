...
The perception layer decides what the event stream means before a renderer styles it. `RendererContextBuilder` folds normalized harn events and touched-file batches into `harn-gibson.world-model.v1`, an event-sourced model of durable facts. The first version tracks per-file activity, bounded structured change facts from tool payloads, command start/result facts, command status/duration when pairable, touched command paths, test/build health facts inferred from command text with observed outcome status, and outcomes with provenance. Later perception enrichers should add richer semantic code graphs and agent attention or intent behind this same renderer-visible contract.
