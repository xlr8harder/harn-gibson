...
To dogfood the renderer-agent process boundary without a live model call, point the server at an external renderer command. The command receives `harn-gibson.external-renderer-request.v1` JSON on stdin and returns a render plan with `steps` on stdout. `examples/renderers/gibson_dogfood_renderer.py` is the hard-coded showcase renderer for live harn sessions; it reacts to event phase, event type, coalesced timing, touched files, and repo topology with a staged scene using the current cinematic primitive/effect set, including a project hologram, data tunnel, signal scope, route trace, repo city, vector sigil, data rain, and persistent effects:
