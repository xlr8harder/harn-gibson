...
Captured `event` and `raw_event` replay steps call the configured renderer live, so the command above dogfoods the hard-coded renderer against the historical agent events. Saved `render_plan` steps are renderer-side fixtures: replay applies the recorded renderer output directly so browser/display changes can be compared against an exact plan. Use `--step-delay-ms` for fixed-delay playback, `--max-step-delay-ms` to cap long real-time gaps, `--no-hold` for automated smoke checks, `--no-browser` when you only want the server URL, and omit `--renderer-command` to compare the deterministic renderer against the same fixture.
