...
To dogfood the renderer-agent process boundary without a live model call, use one of the launcher presets or point the server at an external renderer command. The command receives `harn-gibson.external-renderer-request.v1` JSON on stdin and returns a render plan with `steps` on stdout. `examples/renderers/gibson1_renderer.py` is the default calmer hard-coded visualizer: it keeps status, a terminal board, depth-2 repo city, repo wire terrain, signal scope, route trace, and low-opacity data rain coherent enough for everyday use while still reacting to event phase, touched files, repo topology, coalesced timing, and the active style pack:

```bash
uv run harn-gibson dogfood
```

There are three integration levels. A renderer decides how events become scene mutations. A primitive/effect expands the visual vocabulary that renderers can target. A display backend consumes scene state and implements that vocabulary in a runtime; the current backend is browser/canvas, but a terminal, native, game-engine, or OpenGL backend can work if it implements the catalog or an advertised subset. `GET /backend-contract` and `uv run harn-gibson backend-contract` expose the endpoint paths, scene/update schema names, core primitive kinds, catalog primitive kinds, effect kinds, mutation ops, input delivery modes, render timing modes, supported style packs, active style pack, and current backend capability profile for that use case. A custom primitive layer can either translate the advertised Gibson catalog to backend-native drawing calls or pair a custom catalog with a renderer that targets it.
...
Run the same `gibson1` fixture under a non-default style pack:
