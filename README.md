...
There are three integration levels. A renderer decides how events become scene mutations. A primitive/effect expands the visual vocabulary that renderers can target. A display backend consumes scene state and implements that vocabulary in a runtime; the current backend is browser/canvas, but a terminal, native, game-engine, or OpenGL backend can work if it implements the catalog or an advertised subset. `GET /backend-contract` and `uv run harn-gibson backend-contract` expose the endpoint paths, scene/update schema names, core primitive kinds, catalog primitive kinds, effect kinds, and current backend declaration for that use case.
